...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        #initiate dict {(m,n): dirt_amount}:
        self.tile_dirt_amount = {}
        for i in range(self.width):
            for j in range(self.height):
                self.tile_dirt_amount[(i, j)] = dirt_amount
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        #change robot pos into tile (m, n)
        m = math.floor(pos.get_x())
        n = math.floor(pos.get_y())
        if self.tile_dirt_amount[(m, n)] - capacity >= 0:
            self.tile_dirt_amount[(m, n)] -= capacity
        else:
            self.tile_dirt_amount[(m, n)] = 0
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        return self.get_dirt_amount(m, n) == 0
    ...
    def get_dirt_amount(self, m, n):
        ...
        Returns: an integer
        """
        return self.tile_dirt_amount[(m, n)]
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        while res == False:
            ...
            if self.is_position_valid(random_pos):
                res = True
        return random_pos
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        new_pos = old_pos.get_new_position(angle, self.speed)
...
#helper function: run 1 trial for num_robots robots
def sim_1trial(num_robots, speed, capacity, width, height, dirt_amount, 
               min_coverage, robot_type):
    anim = ps3_visualize.RobotVisualization(num_robots, width, height, True)
    room = FurnishedRoom(width, height, dirt_amount)
    room.add_furniture_to_room()
    robot_dict = {}
    for i in range(1, num_robots + 1):
        robot_dict[i] = robot_type(room, speed, capacity)
    #number of tiles to be cleaned:
    min_tiles = min_coverage*room.get_num_tiles()
    steps = 0
    while room.get_num_cleaned_tiles() < min_tiles:
        for key in robot_dict:
            robot_dict[key].update_position_and_clean()
            anim.update(room, robot_dict.values())
            steps += 1
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, 
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                FaultyRobot)
    """
    step_list = [0]*num_trials
    for i in range(num_trials):
        steps = sim_1trial(num_robots, speed, capacity, width, 
